...
static bool blobfs_cache_pool_need_reclaim(void);
static int reclaim_cache_buffers(struct spdk_file *file);
...
static int
fs_channel_create(struct spdk_filesystem *fs, struct spdk_fs_channel *channel, uint32_t max_ops)
{
	uint32_t i;
...
static int _handle_deleted_files(struct spdk_fs_request *req);
...
void spdk_file_sync_async(struct spdk_file *file, struct spdk_io_channel *_channel,
						  spdk_file_op_complete cb_fn, void *cb_arg)
{
	struct spdk_fs_channel *channel = spdk_io_channel_get_ctx(_channel);
