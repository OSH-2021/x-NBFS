...
static void nbfs2_read_file(struct _nbfs2_args *args, void *buffer,
                            uint64_t length)
{
    //read file to buffer(must be opened)
    read_normal_blob(args->fs->bs, args->file->blobid, buffer,
                     ENABLE_RANDOM_RW, length, NULL, NULL);
}
static void nbfs2_write_file(struct _nbfs2_args *args, void *buffer,
                             uint64_t length)
{
...
static void nbfs2_open_file(struct _nbfs2_args *args, char *filename)
{
