...
static void write_super_blob(struct spdk_blob_store *bs, void *payload, uint64_t offset,
                             uint64_t length, spdk_blob_op_complete cb_fn, void *cb_arg);
static void _wsb_write_complete(void *cb_arg, int bserrno)
{
    //this function runs after spdk_blob_io_write
    SPDK_NOTICELOG("entry\n");
    struct _write_super_blob_args *args = cb_arg;
    //errno handle
    if (bserrno)
    {
...
static void _wsb_sync_complete(void *cb_arg, int bserrno)
{
    ...
    spdk_blob_io_write(args->blob, args->channel, args->payload, args->offset,
                       args->length, _wsb_write_complete, args);
}
static void _wsb_resize_complete(void *cb_arg, int bserrno)
{
    ...
    SPDK_NOTICELOG("super blob now has USED clusters of %" PRIu64 "\n",
                   c_num);
    //synchronize
    spdk_blob_sync_md(args->blob, _wsb_sync_complete, args);
}
static void
_wsb_open_complete(void *cb_arg, struct spdk_blob *blb,
                   int bserrno)
{
    ...
    SPDK_NOTICELOG("super blob now has USED io_units of %" PRIu64 "\n",
                   io_unit_num);
    ...
    if (args->offset + args->length > io_unit_num)
    {
        ...
        //resize the superblob
        spdk_blob_resize(args->blob, c_num + add_c_num, _wsb_resize_complete, args);
    }
    else
    {
        //write data
...
static void _wsb_super_blobid_got(void *cb_arg, spdk_blob_id blobid,
                                  int bserrno)
{
    ...
    spdk_bs_open_blob(args->bs, args->blobid,
                      _wsb_open_complete, args);
}
/*write data in payload(buffer) to super blob */
static void
write_super_blob(struct spdk_blob_store *bs, void *payload, uint64_t offset,
                 uint64_t length, spdk_blob_op_complete cb_fn, void *cb_arg)
{ //this function open a super blob and an io channel and write data
    SPDK_NOTICELOG("entry\n");
    struct _write_super_blob_args *args = NULL;
    if ((args = malloc(sizeof(struct _write_super_blob_args))) == NULL)
    {
        cb_fn(cb_arg, -ENOMEM);
        SPDK_ERRLOG("Could not malloc _write_super_blob_args struct!!\n");
        return;
    }
    args->bs = bs;
    ...
    //get super blob id
    spdk_bs_get_super(bs, _wsb_super_blobid_got, args);
}